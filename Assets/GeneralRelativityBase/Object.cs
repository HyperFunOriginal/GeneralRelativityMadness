...
public class Object : MonoBehaviour
{
    ...
    public float[,] GetEnergyMomentumTensorAround()
    {
        RicciTensors curvature = GetCurvatureAround();
        float kFactor = 8f * Mathf.PI / (global.sqC * global.sqC);
        float ricciScalar = curvature.RicciScalar(currentSpace);
        float[,] tensor = new float[4, 4];
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                tensor[i, j] = (curvature.ricciTensor[i, j] - 0.5f * ricciScalar * currentSpace.components[i, j]) / kFactor;
        return tensor;
    }
    bool Sanity()
    {
    ...
    internal virtual void ProperClockTick()
    {
